      ...
      <form @submit.prevent="handleBinding" class="binding-form-content">
        ...
        <div class="auto-verify-note">
          ...
          <div class="verify-text">
            <p><strong>{{ $t('staffBinding.autoVerifyTitle') }}</strong></p>
            <p>{{ $t('staffBinding.autoVerifyDesc') }}</p>
          </div>
        </div>

        <div class="line-login-section">
          <p class="line-login-note">{{ $t('staffBinding.autoVerifyNote') }}</p>
          <button
            type="button"
...
import liffServiceSecure from '../services/liffServiceSecure'
...
async function initializeLiff() {
  try {
    ...
    // 注意：现在使用liffServiceSecure而非window.liff

    // 获取门店信息
    await fetchStoreInfo()

    // 如果已经在LINE中登录，自动处理绑定
    if (liffServiceSecure.isLoggedIn()) {
...
async function initiateLineLogin() {
  try {
    ...
    if (!liffServiceSecure.isLoggedIn()) {
      loadingMessage.value = t('staffBinding.lineLoginButton')
      // 使用带手机号权限的登录方法
      await liffServiceSecure.loginWithPhoneScope()
    } else {
...
async function processBinding() {
  try {
    ...
    // 获取ID Token
    const idToken = liffServiceSecure.getIdToken()
    if (!idToken) {
      throw new Error('无法获取LINE身份信息')
    }

    // 调用绑定API（手机号自动从LINE ID Token获取）
...
function closeWindow() {
  if (liffServiceSecure.isInClient()) {
    liffServiceSecure.closeWindow()
  } else {
    window.close()
  }
}
