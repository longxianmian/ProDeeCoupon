...
const { coupons, stores, posts } = require('../../shared/schema');
...
// 即时翻译内容（用于活动创建时） - 需要管理员权限
router.post('/translate-content', adminAuth, async (req, res) => {
  try {
...
// 翻译视频内容 - 需要管理员权限
router.post('/post/:id', adminAuth, async (req, res) => {
  try {
    const postId = parseInt(req.params.id);
    
    // 获取视频内容数据
    const postData = await db.select().from(posts).where(eq(posts.id, postId)).limit(1);
    ...
    await db.update(posts)
      .set({
        ...
        title_en_us: translations.title_en_us,
        title_th_th: translations.title_th_th,
        content_zh_cn: translations.content_zh_cn,
        content_en_us: translations.content_en_us,
        content_th_th: translations.content_th_th,
        updated_at: new Date()
      })
...
// 批量翻译所有已发布视频 - 需要管理员权限
router.post('/batch/posts', adminAuth, async (req, res) => {
  try {
    // 获取所有已发布的内容
    ...
    for (const post of allPosts) {
      try {
        // 检查是否已经有翻译
        if (post.title_en_us && post.title_th_th) {
          console.log(`⏭️  内容 ${post.id} 已有翻译，跳过`);
          continue;
        }

        console.log(`🔄 翻译内容 ${post.id}: ${post.title}`);
        ...
        console.error(`🚨 内容 ${post.id} 翻译失败:`, error.message);
        failed++;
